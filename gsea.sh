...
#  Here: $2 = HUGO gene symbol, $5 = logFC, $9 = adjusted p-value   
awk -v OFS='\t'	'{LOG=log($9)/log(10); print($2,LOG)}' ${IN%.txt}_geneSymbols.txt > ${IN%.txt}_forGSEA_combined.rnk
awk -v OFS='\t'	'function sign(v) {return v < 0 ? -1 : 1} {LOG=log($9)/log(10); print($2,LOG*sign($5))}' ${IN%.txt}_geneSymbols.txt > ${IN%.txt}_forGSEA_directed.rnk
...
do
    set=$1
    java -cp /NGS/gsea/gsea2-2.2.1.jar -Xmx512m xtools.gsea.GseaPreranked -gmx $set -collapse false -mode Max_probe -norm meandiv -nperm 1000 -rnk ${IN%.txt}_forGSEA_combined.rnk -scoring_scheme weighted -rpt_label "$(basename ${IN})_$(basename ${set})_" -include_only_symbols true -make_sets true -plot_top_x 20 -rnd_seed timestamp -set_max 500 -set_min 15 -zip_report false -gui false
    java -cp /NGS/gsea/gsea2-2.2.1.jar -Xmx512m xtools.gsea.GseaPreranked -gmx $set -collapse false -mode Max_probe -norm meandiv -nperm 1000 -rnk ${IN%.txt}_forGSEA_directed.rnk -scoring_scheme weighted -rpt_label "$(basename ${IN})_$(basename ${set})_" -include_only_symbols true -make_sets true -plot_top_x 20 -rnd_seed timestamp -set_max 500 -set_min 15 -zip_report false -gui false
    shift 1
done
