...
P1_LS1="$( awk '$3 == "+" && $14 ~ /p/ {print $12}' "$FILE" | sed -e 's/[MDpN]/\t/g' -e 's/[0-9]\+[SHI]//g' | awk -v OFS='\t'  '{SUM=$1; for(i=2;i<=NF;i++)SUM=SUM+$i; print SUM}')"
...
P2_LS2="$( awk '$3 == "+" && $12 ~ /p/ {print $14}' "$FILE" | sed -e 's/[MDpN]/\t/g' -e 's/[0-9]\+[SHI]//g' | awk -v OFS='\t'  '{SUM=$1; for(i=2;i<=NF;i++)SUM=SUM+$i; print SUM}')"
